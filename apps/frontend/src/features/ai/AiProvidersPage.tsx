...
import { api, type ProvedorLlm } from "../../api/client";

export function AiProvidersPage() {
  const [providers, setProviders] = useState<ProvedorLlm[]>([]);
    ...
    {loading ? <Box sx={{ py: 8, display: "grid", placeItems: "center" }}><CircularProgress size={28} /></Box> : <Stack spacing={1.5}>
      {providers.map((provider) => <Paper key={provider.id} elevation={0} sx={{ p: 2.5, border: "1px solid", borderColor: "divider" }}>
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.2fr 1.5fr 1fr auto" }, gap: 2.5, alignItems: "center" }}>
          <Stack direction="row" spacing={1.5} alignItems="center"><Box sx={{ width: 40, height: 40, borderRadius: 2.5, display: "grid", placeItems: "center", bgcolor: "primary.light", color: "primary.main" }}><MemoryRounded /></Box><Box><Typography fontWeight={750}>{provider.nome}</Typography><Chip size="small" color={provider.habilitado ? "success" : "default"} label={provider.habilitado ? "Ativo" : "Desativado"} sx={{ mt: .6 }} /></Box></Stack>
          <Box><Typography variant="body2" color="text.secondary">Endpoint</Typography><Typography variant="body2" fontWeight={650} sx={{ overflowWrap: "anywhere" }}>{provider.urlBase}</Typography></Box>
          <Box><Typography variant="body2" color="text.secondary">Modelo padrão</Typography><Typography variant="body2" fontWeight={650}>{provider.modelo}</Typography></Box>
